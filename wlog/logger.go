...
type LoggerCreator[T wtypes.LogTypes] func(w io.Writer) Logger[T]
...
type Logger[T wtypes.LogTypes] interface {
	Log(*T)
}
...
func NewDefaultLogger[T wtypes.LogTypes](output io.Writer) Logger[T] {
...
func NewDefaultLoggerWithLoggerProvider[T wtypes.LogTypes](provider LoggerProvider) LoggerCreator[T] {
...
func NewDefaultLoggerWithPrinter[T wtypes.LogTypes](printer Printer) Logger[T] {
	return &defaultLogger[T]{
		printer: printer,
	}
}

type defaultLogger[T wtypes.LogTypes] struct {
	printer Printer
}

func (l *defaultLogger[T]) Log(log *T) {
	err := l.printer.Print(any(*log).(wtypes.LogType))
	if err != nil {
