...
type defaultLogger struct {
	logger       wlog.ConjureLogger[logging.RequestLogV2]
...
func ToParams(r Request, idsExtractor extractor.IDsFromRequest, pathParamPerms, queryParamPerms, headerParamPerms ParamPerms) []Param {
	...
	return []Param{
		...
		RequestSize(r.Request.ContentLength),
		ResponseSize(r.ResponseSize),
		Duration(r.Duration),
		UID(idsMap[extractor.UIDKey]),
		SID(idsMap[extractor.SIDKey]),
		TokenID(idsMap[extractor.TokenIDKey]),
		OrgID(idsMap[extractor.OrgIDKey]),
		TraceID(idsMap[traceIDKey]),
...
func parseRequestParams(r Request, pathParamPerms, queryParamPerms, headerParamPerms ParamPerms) (safeParams, unsafeParams map[string]any) {
